...
class AppShadingNetwork(nn.Module):
    ...
    def __init__(self, cfg, bound=1, hash_encoding=False, fp16=False):
        ...
        if self.hash_encoding:
            ...
            self.albedo_predictor = make_predictor(in_dim_material + 3, 3, 256, 4)
            self.metallic_predictor = make_predictor(in_dim_material + 3, 1, 256, 4)
            if self.cfg['metallic_init'] != 0:
                nn.init.constant_(self.metallic_predictor[-2].bias, self.cfg['metallic_init'])
            self.roughness_predictor = make_predictor(in_dim_material + 3, 1, 256, 4)
