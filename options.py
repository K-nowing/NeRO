# The code is from https://github.com/chenhsuanlin/bundle-adjusting-NeRF
...
def set(opt_path, opt_cmd={}):
    logger.info("setting configurations...")
    opt_base = load_options(opt_path)
...
def process_options(opt):
    ...
    if opt.seed is not None:
        ...
        if opt.seed!=0:
            opt.exp_name = str(opt.exp_name)+"_seed{}".format(opt.seed)
    else:
        # create random string as run ID
        randkey = "".join(random.choice(string.ascii_uppercase) for _ in range(4))
        opt.exp_name = str(opt.exp_name)+"_{}".format(randkey)
    # other default options
    opt.output_path = "{0}/{1}/{2}/{3}/{4}".format(opt.output_root, opt.group, opt.database_name, opt.network, opt.exp_name)
    os.makedirs(opt.output_path, exist_ok=True)
    assert(isinstance(opt.gpu, int)) # disable multi-GPU support for now,  single is enough
    opt.device = "cpu" if opt.gpu == -1 or not torch.cuda.is_available() else "cuda:{}".format(opt.gpu)
